...
def build_vocab(path, data):
    ...
    embed = []
    embed_dic = {}
    with open('./data/vector.txt', 'r') as vector_file:
        lines = vector_file.readlines()
        for line in lines:
            line_list = line.split(' ')
            embed_dic[line_list[0]] = line_list[1:]
    for voc in vocab_list:
        if voc in embed_dic.keys():
            embed.append(embed_dic[voc] + [0.] * (FLAGS.embed_units - len(embed_dic[voc])))
        else:
